<template>
  <section class="center" style="margin-bottom: 30px; width: 100%;">
    <div class="border-grey bg-white" style="width: 96%; max-height: 770px;">
      <div class="df df-jcsa df-aic bottomline-grey f-t2" style="height: 56px;"><span style="width: 96%;">Inspection Forms</span></div>
        ...
        <article class="bottomline-grey df df-jcfe" v-for="(e, i) in list" :key="i">
          <section class="df df-aic" style="width: 96%; height: 48px;">
            ...
            <div :class="{smallHidden: columnList[1].smallHidden}" class="df df-aic" :style="{flex: columnList[1].flex}">
              ...
              <div style="margin-left: 20px;">{{e.content || 'No Detail Reports'}}</div>
            </div>
            <div :class="{smallHidden: columnList[2].smallHidden}" :style="{flex: columnList[2].flex}">{{e.date | date}}</div>
            <div :class="{smallHidden: columnList[3].smallHidden}" class="df df-aic" :style="{flex: columnList[3].flex}">
              ...
              <img v-if="e.status === 'COMPLETE'" src="@/assets/img/greenDot.png" alt="" style="width: 12px; height: 12px;">
...
export default {
  data() {
    return {
      ...
      columnList: [
        { name: 'Title', flex: 1 },
        { name: 'Details', flex: 1, smallHidden: true },
  ...
  computed: {
    list() { return this.$store.state.vehicle.inspection.list },
    page() { return this.$store.state.vehicle.inspection.page },
  },
  ...
  methods: {
    ...
    async  loadInspection() {
      this.loading = true
      await Promise.all([this.$store.dispatch('vehicle/loadInspection', { id: this.$route.params.id }), this.$wait(1000)])
      this.loading = false
    },
  },
}
